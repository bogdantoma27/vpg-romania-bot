...
async function fetchCurrentWeek(season) {
  const raw  = await vpgGet(`/table/?season=${season}&is_history=false`);
  const list = Array.isArray(raw) ? raw : (raw?.data ?? raw?.results ?? []);
  if (!list.length) return 1;
  const maxPlayed = Math.max(...list.map(r => Number(r.played ?? 0)));
  return Math.max(1, Math.ceil(maxPlayed / 2));
}
...
async function fetchAllLeaderboards(season, weekly) {
  ...
  POSITIONS.forEach((pos, i) => {
    lb[pos] = results[i].status === 'fulfilled' ? results[i].value : [];
  });
  return lb;
}
